...
def create_form(parent):
    ...
    widget_holder = [first_row, second_row, third_row, fourth_row, fifth_row]
    ...
    for row in widget_holder:
        ...
        for index, (key, value) in enumerate(row.items()):
            ...
            if len(value) == 3:
                value[1]['width'] = value[2]
                value[1]['justify'] = 'right'
                if key == 'Sexo':
                    value[1]['justify'] = 'left'
                elif key == 'Nacimiento':
                    value[1]['justify'] = 'center'
                value[1].pack(side='left', padx= (0,5), pady=5)
            else:
                value[1].pack(side='left', expand= 1, fill='x', padx= (0,5), pady=5)
...
def OpenFiles(form):
    ...
    form_matrix = [*viewed_horse, father_object, mother_object]
